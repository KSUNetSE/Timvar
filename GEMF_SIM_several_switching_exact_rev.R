GEMF_SIM_several_switching_exact_rev<-function(Para,IN1,IN2,Nei1,Nei2,x0,maxNumevent,Runtime,N,numrun,res){
 
  #----------------------------------------------------------------added in rev
  Tr<-seq(0,Runtime,res);
  #----------------------------------------------------------------added in rev
...
for(n in 1:N){
 ...
 di<-matrix(0,M,1);
  ...
  lnTr=length(Tr);
  S1pop<-matrix(as.integer(0),lnTr,numrun);
  #S1pop[1,]=length(x0[x0==1]);
  
  S2pop<-matrix(as.integer(0),lnTr,numrun);
  #S2pop[1,]=length(x0[x0==2]);
  
  I1pop<-matrix(as.integer(0),lnTr,numrun);
  #I1pop[1,]=length(x0[x0==3]);
  
  I2pop<-matrix(as.integer(0),lnTr,numrun);
  #I2pop[1,]=length(x0[x0==4]);
 ...
 #-----------------------------------------    
      ...
      if (is==4 && js==2){#I2 to s2
        ...
        R<-max(R-Rnvar[ns],0);
        Rnvar[ns]<<-di[js]+h2var[ns]*beta2+h1var[ns]*beta1;
        R<-R+Rnvar[ns];
  ...
  # #----------------------------------------------------------------added in rev
  f2<-function(rundm){
    Tdm<-c(0,cumsum(ts[1:lasteventnumber]));
    
    dumdm=c(length(x0[x0==1]),length(x0[x0==2]),length(x0[x0==3]),length(x0[x0==4]));
    ...
    for(i in 1:lasteventnumber){
      for(j in mj:(lnTr)){
        if(Tdm[i]<=Tr[j]&&Tr[j]<Tdm[i+1]){
          
        
          S1pop[j,rundm]<<-dumdm[1];
          S2pop[j,rundm]<<-dumdm[2];
          I1pop[j,rundm]<<-dumdm[3];
          I2pop[j,rundm]<<-dumdm[4];

          lif<-j;
          
          };
        if(Tdm[i+1]<=Tr[j]){mj<-j;break;}
      }
      
      dumdm[j_index[i]]=dumdm[j_index[i]]+as.integer(1);
      dumdm[i_index[i]]=dumdm[i_index[i]]-as.integer(1);
      
    };
    
     if(lif<lnTr){
      
      S1pop[(lif+1):lnTr,rundm] <<- as.integer(-1);
      S2pop[(lif+1):lnTr,rundm] <<- as.integer(-1);
      I1pop[(lif+1):lnTr,rundm] <<- as.integer(-1);
      I2pop[(lif+1):lnTr,rundm] <<- as.integer(-1);
  ...
  for(i in 1:(numrun)){
    ...
    if(lasteventnumber>0){f2(i);};
    print(lasteventnumber);
    print(i);};
 ...
 ############ 
  ...
  list(Tr,S1pop,S2pop,I1pop,I2pop);}
